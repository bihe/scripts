...
if [[ -z $MONGODB_USER ]] || [[ -z $MONGODB_PASSWORD ]] || [[ -z $AWS_ACCESS_KEY ]] || [[ -z $AWS_SECRET_KEY ]] || [[ -z $S3_REGION ]] || [[ -z $S3_BUCKET ]] || [[ -z $MONGODB_PORT ]]
...
mongo -u "$MONGODB_USER" -p "$MONGODB_PASSWORD" --port $MONGODB_PORT admin --eval "var databaseNames = db.getMongo().getDBNames(); for (var i in databaseNames) { printjson(db.getSiblingDB(databaseNames[i]).getCollectionNames()) }; printjson(db.fsyncLock());"
echo "--- DATABASE LOCKED ---"
# Dump the database
mongodump -u "$MONGODB_USER" -p "$MONGODB_PASSWORD" -h localhost:$MONGODB_PORT  --out $DIR/backup/$FILE_NAME
echo "--- DATABASE DUMPED ---"
# Unlock the database
mongo -u "$MONGODB_USER" -p "$MONGODB_PASSWORD" --port $MONGODB_PORT admin --eval "printjson(db.fsyncUnlock());"
